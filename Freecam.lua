...
local TELEPORT_MACRO_KB = {Enum.KeyCode.LeftShift, Enum.KeyCode.L}
	...
	elseif _G.GluecamEnabled == true then
		if _G.GluecamTarget:FindFirstChild("Character") then
			if _G.GluecamTarget.Character:FindFirstChild("Head") then
				cameraCFrame = CFrame.new(_G.GluecamTarget.Character.Head.CFrame.Position)*CFrame.fromOrientation(cameraRot.x, cameraRot.y, 0)*CFrame.new(vel*NAV_GAIN*dt)
			else
				print("So, no head?")
			end
		else
...
do
	...
	local function TeleportPlayer()
		game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position = game.Workspace.CurrentCamera.CFrame.Position
	end
	...
	local function HandleTeleportInput(action, state, input)
		if state == Enum.UserInputState.Begin then
			if input.KeyCode == TELEPORT_MACRO_KB[#TELEPORT_MACRO_KB] then
				local bruh = true
				for i = 1, #TELEPORT_MACRO_KB - 1 do
					if not UserInputService:IsKeyDown(TELEPORT_MACRO_KB[i]) then
						bruh = false
					end
				end
				if bruh == true then
	...
	ContextActionService:BindActionAtPriority("TeleportPlayer", HandleTeleportInput, false, TOGGLE_INPUT_PRIORITY, TELEPORT_MACRO_KB[#TELEPORT_MACRO_KB])
end
